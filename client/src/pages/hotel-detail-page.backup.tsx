...
export default function HotelDetailPage() {
  ...
  // Tarih değerlerini güvenlik kontrolü ile oluştur
  ...
  const handleRoomSelect = (roomId: number) => {
    ...
    // Log route we're trying to navigate to
    console.log(`Redirecting to: /reservations/new?roomId=${roomId}`);
    // Use window.location instead of wouter's setLocation for query params
        ...
        <section className="py-3" id="booking">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-sm overflow-hidden">
              {/* Arama formu - Basitleştirilmiş tasarım */}
              <div className="p-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                  <div className="border border-border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <div>
                        <div className="text-xs text-neutral-500 mb-1">Giriş Tarihi</div>
                        <div className="font-medium text-primary">
                          {format(dateRange.from!, 'd MMMM yyyy', { locale: tr })}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs text-neutral-500 mb-1">Çıkış Tarihi</div>
                        <div className="font-medium text-primary">
                    ...
                    <div className="text-xs mb-2">
                      <span className="font-medium">Toplam:</span> {nightCount} Gece
                    </div>
                  </div>
                  ...
                  <div className="border border-border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div>
                        <div className="text-xs text-neutral-500 mb-1">Oda</div>
                        <div className="flex items-center">
                          <div className="font-medium">{roomCount}</div>
                          <div className="flex ml-2">
                            <button 
                              onClick={() => setRoomCount(prev => Math.min(prev + 1, 5))}
                              className="text-xs px-1.5 py-0.5 bg-neutral-100 dark:bg-neutral-700 rounded-sm hover:bg-neutral-200 dark:hover:bg-neutral-600 ml-1"
                            >
                              +
                            </button>
                            <button 
                              onClick={() => setRoomCount(prev => Math.max(prev - 1, 1))}
                              className="text-xs px-1.5 py-0.5 bg-neutral-100 dark:bg-neutral-700 rounded-sm hover:bg-neutral-200 dark:hover:bg-neutral-600 ml-1"
                      ...
                      <div>
                        <div className="text-xs text-neutral-500 mb-1">Yetişkin</div>
                        <div className="flex items-center">
                          <div className="font-medium">{adultCount}</div>
                          <div className="flex ml-2">
                            <button 
                              onClick={() => setAdultCount(prev => Math.min(prev + 1, 10))}
                              className="text-xs px-1.5 py-0.5 bg-neutral-100 dark:bg-neutral-700 rounded-sm hover:bg-neutral-200 dark:hover:bg-neutral-600 ml-1"
                            >
                              +
                            </button>
                      ...
                      <div>
                        <div className="text-xs text-neutral-500 mb-1">Çocuk</div>
                        <div className="flex items-center">
                          <div className="font-medium">{childCount}</div>
                          <div className="flex ml-2">
                            <button 
                              onClick={() => setChildCount(prev => Math.min(prev + 1, 5))}
                              className="text-xs px-1.5 py-0.5 bg-neutral-100 dark:bg-neutral-700 rounded-sm hover:bg-neutral-200 dark:hover:bg-neutral-600 ml-1"
                            >
                              +
                            </button>
                ...
                <Button 
                  ...
                  onClick={() => {
                    // Odaları sayfada göstermek yerine seçilen tarihlerle birlikte rezervasyon sayfasına yönlendir
                    const checkIn = format(dateRange.from!, 'yyyy-MM-dd');
                    const checkOut = format(dateRange.to!, 'yyyy-MM-dd');
                    window.location.href = `/hotel/${id}/rooms?checkIn=${checkIn}&checkOut=${checkOut}&adults=${adultCount}&children=${childCount}&rooms=${roomCount}`;
                  }}
                >
                  Odaları Görüntüle
                </Button>
              ...
              <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-sm p-4">
                ...
                <div className="rounded-lg overflow-hidden border border-border">
                  <Calendar
                    ...
                    modifiers={{
                      booked: [(date) => {
                        // Örnek dolu günler - bu fonksiyon gerçekte API'den gelen bilgilerle dolacak
                        const bookedDates = [
                          new Date(2025, 4, 10), // 10 Mayıs
                          new Date(2025, 4, 11), // 11 Mayıs
                          new Date(2025, 4, 15), // 15 Mayıs
                          new Date(2025, 4, 16), // 16 Mayıs
                        ];
                ...
                <div className="mt-3 text-xs text-neutral-500 dark:text-neutral-400">
                  <ul className="space-y-1">
                    <li className="flex items-center">
                      ...
                      <span>Mavi: Seçilen tarihler</span>
                    </li>
                    <li className="flex items-center">
                      <div className="h-3 w-3 rounded-full bg-red-100 mr-2"></div>
                      <span>Kırmızı: Dolu tarihler</span>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-sm p-4">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium">{format(dateRange.from!, 'dd MMMM yyyy', { locale: tr })}</span>
                  </div>
                  
                  <div className="flex justify-between items-center">
                    <span>Çıkış Tarihi:</span>
                    <span className="font-medium">{format(dateRange.to!, 'dd MMMM yyyy', { locale: tr })}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    <span>Misafir:</span>
                    <span className="font-medium">{adultCount} Yetişkin, {childCount} Çocuk</span>
                  </div>
                  
                  <Separator />
                  
                  {/* Dinamik fiyat bilgileri - Tarih seçimine göre değişiyor */}
                  <div className="space-y-2">
                    {Array.from({ length: nightCount }).map((_, index) => {
                      const currentDate = new Date(dateRange.from!);
                      ...
                      return (
                        <div key={index} className="flex justify-between items-center text-sm">
                          <span>{format(currentDate, 'dd MMMM yyyy', { locale: tr })}</span>
                          <span className="font-medium">{dayPrice.toLocaleString('tr-TR')} ₺</span>
                        </div>
                      );
                    })}
                  </div>
