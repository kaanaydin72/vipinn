...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (savedTheme && Object.values(THEMES).includes(savedTheme as THEMES)) {
      updateTheme(savedTheme as THEMES);
    } else if (siteTheme && 'theme' in siteTheme) {
      // Yerel tercih yoksa, site temasını kullan
      const themeValue = siteTheme.theme;
      // ThemeType olarak geçerli mi kontrol et
      const validTheme = Object.values(THEMES).includes(themeValue as THEMES)
        ? themeValue as THEMES
        : THEMES.CLASSIC;
